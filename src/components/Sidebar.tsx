...
import { 
    SearchIcon, HomeIcon, CheckSquareIcon, CalendarIcon, UsersIcon, FileTextIcon, 
...
const NavItem = ({ icon, children, active = false, isOpen }: { icon: React.ReactNode; children: React.ReactNode; active?: boolean; isOpen: boolean }) => (
  <a href="#" className={`flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${active ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-100'} ${!isOpen && 'justify-center'}`}>
    {icon}
...
const CollapsibleSection = ({ title, children, defaultOpen = false, isOpen }: { title: string; children?: React.ReactNode; defaultOpen?: boolean; isOpen: boolean; }) => {
  ...
  // Jika sidebar ditutup, tampilkan hanya ikon dari children tanpa fungsionalitas collapse
  if (!isOpen) {
    return (
      <div className="pt-2">
        {children}
...
const ProjectLink = ({ color, name, isOpen }: { color: string; name: string; isOpen: boolean }) => (
    <a href="#" className={`flex items-center px-3 py-2 text-sm font-medium text-gray-600 rounded-md hover:bg-gray-100 ${!isOpen && 'justify-center'}`}>
    ...
    <aside className={`bg-gray-50/50 border-r border-gray-200/50 flex flex-col transition-all duration-300 ease-in-out ${isOpen ? 'w-64 p-4 space-y-4' : 'w-20 p-2 space-y-2'}`}>
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto">
        <div className="space-y-1">
          ...
          <NavItem icon={<HomeIcon className="w-5 h-5" />} active={true} isOpen={isOpen}>Home</NavItem>
          <NavItem icon={<CheckSquareIcon className="w-5 h-5" />} isOpen={isOpen}>Tasks</NavItem>
          <NavItem icon={<CalendarIcon className="w-5 h-5" />} isOpen={isOpen}>Calendar</NavItem>
          <NavItem icon={<UsersIcon className="w-5 h-5" />} isOpen={isOpen}>Teams</NavItem>
          <NavItem icon={<FileTextIcon className="w-5 h-5" />} isOpen={isOpen}>Docs</NavItem>
          <NavItem icon={<BotIcon className="w-5 h-5" />} isOpen={isOpen}>Automations</NavItem>
          <NavItem icon={<BarChart2Icon className="w-5 h-5" />} isOpen={isOpen}>Reporting</NavItem>
        </div>

        <div className="border-t border-gray-200/80">
          <CollapsibleSection title="Projects" defaultOpen={true} isOpen={isOpen}>
             <ProjectLink color="#63d393" name="Atlas CRM Revamp" isOpen={isOpen} />
             <ProjectLink color="#f19953" name="Nimbus Dashboard" isOpen={isOpen} />
             <ProjectLink color="#4f80e1" name="Orion API Gateway" isOpen={isOpen} />
             <ProjectLink color="#e14f82" name="Helio Task System" isOpen={isOpen} />
          </CollapsibleSection>
        </div>
        ...
        <div className="border-t border-gray-200/80">
          <CollapsibleSection title="Support" defaultOpen={true} isOpen={isOpen}>
              <NavItem icon={<SettingsIcon className="w-5 h-5" />} isOpen={isOpen}>Settings</NavItem>
              <NavItem icon={<GitMergeIcon className="w-5 h-5" />} isOpen={isOpen}>Releases</NavItem>
          </CollapsibleSection>
        </div>
      </nav>
      ...
      <div className="mt-auto border-t border-gray-200/80">
        <CollapsibleSection title="Apps" defaultOpen={true} isOpen={isOpen}>
            <NavItem icon={<TrelloIcon className="w-5 h-5" />} isOpen={isOpen}>Trello</NavItem>
            <NavItem icon={<FigmaIcon className="w-5 h-5" />} isOpen={isOpen}>Figma</NavItem>
        </CollapsibleSection>
      </div>
    </aside>
  );
